...
int main3(void){ // main3
  ...
  while(1){
    ...
    //DAC5_Out(1);   // only PB0, expect 0.11V
    //Clock_Delay1ms(2000);
...
int main(void){// main6
  ...
  while(1){
    ...
    if(key != last){
      if(key == 0){
        Sound_Stop();
      } else if(key & 0x01){    // Key0 = 220.0 Hz
        Sound_Start(11364);
        Debug_Dump(11364);
      } else if(key & 0x02){     // Key1 = 277.2 Hz
        Sound_Start(9017);
        Debug_Dump(9017);
      } else if(key & 0x04){   // Key2 = 329.6 Hz
        Sound_Start(7580);
        Debug_Dump(7580);
      } else if(key & 0x08){  // Key3 = 370.0 Hz
        Sound_Start(6757);
        Debug_Dump(6757);
      }
      last = key;
...
void Sound_Init(uint32_t period, uint32_t priority){
  Index = 0;
    ...
    SysTick->VAL  = 0;      // any write clears current value
     SCB->SHP[1] = SCB->SHP[1]&(~0xC0000000)|(priority << 30); // set priority = 1
    SysTick->CTRL = 0x0007;        // enable SysTick, core clock, interrupts
}
void Sound_Stop(void){
...
void Sound_Start(uint32_t period){
    SysTick->LOAD = period - 1; // set new period
    SysTick->VAL  = 0;          // restart counter
    Index = 0;                  // reset to beginning of sine table
}
...
void SysTick_Handler(void){
    ...
    DAC5_Out(SinWave[Index]); // output current sine value to DAC
    Index = (Index + 1) & 0x1F; // increment and wrap 0-31 
}
